...
def _get_block_id_from_answer(item: dict) -> str:
...
def build_pdf_report(user_id: int, results: dict, answers: list[dict]) -> str:
    ...
    _write_text(
        pdf,
        "Итоговый отчёт по диагностике типов мышления",
        size=13,
        bold=True,
        line_height=8,
        align="C",
    ...
    _write_text(
        ...
        (
            ...
            "какие направления требуют развития, какие выводы можно сделать "
            "и как использовать результаты в реальной жизни."
        ),
        size=10,
        line_height=7,
    )
    ...
    _write_text(
        ...
        (
            ...
            "какие зоны требуют развития и какие шаги можно предпринять дальше."
        ),
        size=10,
        line_height=7,
    )
    ...
    _write_text(
        pdf,
        f"Всего проверено типов мышления: {summary['total']}",
        size=11,
    )
    _write_text(
        pdf,
        f"Найдено типов мышления: {summary['found']}",
        size=11,
    )
    _write_text(
        pdf,
        f"Зоны развития: {summary['missing']}",
        size=11,
    )
    _write_text(
        pdf,
    ...
    _write_text(pdf, diagnostic_text, size=10, line_height=7)
    ...
    _section_title(pdf, "6. Рекомендации")
    ...
    _write_text(
        ...
        (
            ...
            "5. Возвращаться к диагностике после обучения или практики, чтобы "
            "сравнить динамику."
        ),
        size=10,
        line_height=7,
    ...
    _write_text(pdf, slogan, size=10, line_height=7)

    # PROJECT DEVELOPMENT
    pdf.add_page()

    _section_title(pdf, "7. Блок для дальнейшего развития проекта")
